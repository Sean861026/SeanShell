...
Preview headers use the cached native application icon without an extra plate
when available and a theme-aware accent tile with a Fluent window glyph
otherwise. This matches the Launcher icon treatment while preserving transparent
brand artwork. Title, process identity, and explicit
